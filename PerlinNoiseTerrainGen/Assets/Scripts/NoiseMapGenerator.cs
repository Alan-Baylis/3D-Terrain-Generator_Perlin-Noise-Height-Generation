...
/*Handles the Generation of the Noise Map*/

public class NoiseMapGenerator : MonoBehaviour {

    // Selection of Draw Modes Available
	...
	Queue<MapThreadInfo<VertexData>> meshDataThreadInfoQueue = new Queue<MapThreadInfo<VertexData>>();
	...
	public void RequestMeshData(MapData mapData, int lod, Action<VertexData> callback) {
		ThreadStart threadStart = delegate {
	...
	void MeshDataThread(MapData mapData, int lod, Action<VertexData> callback) {
		VertexData meshData = MeshGenerator.GenerateTerrainMesh (mapData.heightMap, meshHeightMultiplier, meshHeightCurve, lod);
		lock (meshDataThreadInfoQueue) {
			meshDataThreadInfoQueue.Enqueue (new MapThreadInfo<VertexData> (callback, meshData));
		}
	}
	...
	void Update() {
		...
		if (meshDataThreadInfoQueue.Count > 0) {
			for (int i = 0; i < meshDataThreadInfoQueue.Count; i++) {
				MapThreadInfo<VertexData> threadInfo = meshDataThreadInfoQueue.Dequeue ();
				threadInfo.callback (threadInfo.parameter);
			}
		}
	}
